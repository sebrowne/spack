...
def padding_filter(string):
    ...
    if not _filter_re:
        ...
        regex = (
            ...
            r"(/{longest_prefix}|/{longest_prefix}{extra_pad_character})?(?=/)"  # trailing prefix of padding as path component
        )
        regex = regex.replace("/", re.escape(os.sep))
        regex = regex.format(pad=pad, extra_pad_character=SPACK_PATH_PADDING_EXTRA_CHAR, longest_prefix=longest_prefix)
